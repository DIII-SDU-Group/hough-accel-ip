...
entity houghlines_accel_BUS_A_s_axi is
generic (
...
end entity houghlines_accel_BUS_A_s_axi;
...
architecture behave of houghlines_accel_BUS_A_s_axi is
    ...
    signal int_img_in_address1 : UNSIGNED(16 downto 0);
    ...
    signal int_img_in_shift    : UNSIGNED(1 downto 0);

    component houghlines_accel_BUS_A_s_axi_ram is
        generic (
            BYTES   : INTEGER :=4;
            DEPTH   : INTEGER :=256;
            AWIDTH  : INTEGER :=8);
    ...
    end component houghlines_accel_BUS_A_s_axi_ram;
...
-- ----------------------- Memory logic ------------------
    ...
    img_in_q0            <= STD_LOGIC_VECTOR(SHIFT_RIGHT(int_img_in_q0, TO_INTEGER(int_img_in_shift) * 8)(7 downto 0));
    int_img_in_address1  <= raddr(18 downto 2) when ar_hs = '1' else waddr(18 downto 2);
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (img_in_ce0 = '1') then
                    int_img_in_shift <= UNSIGNED(img_in_address0(1 downto 0));
                end if;
            end if;
        end if;
    end process;
...
entity houghlines_accel_BUS_A_s_axi_ram is
    generic (
        BYTES   : INTEGER :=4;
        DEPTH   : INTEGER :=256;
        AWIDTH  : INTEGER :=8);
...
end entity houghlines_accel_BUS_A_s_axi_ram;

architecture behave of houghlines_accel_BUS_A_s_axi_ram is
